...
public class MakiyamaExtended {

	
	private TreeMap<String, Integer> itemList = null;
	
	/*
	public TreeMap<String, Integer> harvestStatement(Statement s1) {
		if (s1 instanceof Select) {
			return harvestSelect((Select)s1);
		}
		//else if (s1 instanceof CreateTable) {
		//	return harvestCreateTable((CreateTable)s1);
		//}
		else if (s1 instanceof Delete) {
			return harvestDelete((Delete)s1);
		}
		//else if (s1 instanceof Drop) {
		//	return harvestDrop((Drop)s1);
		//}
		else if (s1 instanceof Insert) {
			return harvestInsert((Insert)s1);
		}
		else if (s1 instanceof Replace) {
			return harvestReplace((Replace)s1);
		}
		//else if (s1 instanceof Truncate) {
		//	return harvestTruncate((Truncate)s1);
		//}
		else if (s1 instanceof Update) {
			return harvestUpdate((Update)s1);
		}       
	}

	private TreeMap<String, Integer> harvestSelect(Select s1) {
		SelectBody selectBody1 = s1.getSelectBody();
		
		return harvestSelectBody(selectBody1);
	}
	
	public TreeMap<String, Integer> harvestSelectBody(SelectBody s1) {
		
		if (s1 instanceof PlainSelect) {
			return harvestPlainSelect((PlainSelect)s1);
		} else if (s1 instanceof Union) {
			return harvestUnion((Union)s1);
		} else {
	...
	private TreeMap<String, Integer> harvestUnion(Union s1) {
	...
	private TreeMap<String, Integer> harvestPlainSelect(PlainSelect p1) {
		
		itemList = new TreeMap<String, Integer>();
		...
		if (d1 != null) {
			addToItemList(harvestDistinct(d1));
		}
	...
	*/
	...
	private void addToItemList(TreeMap<String, Integer> treeMap) {
		...
		while(it.hasNext()) {
			...
			if (itemList.containsKey(temp)) {
				itemList.put(temp, itemList.get(temp) + treeMap.get(temp));
			} else {
				itemList.put(temp, 1);
			}
		}
	...
	private TreeMap<String, Integer> harvestDistinct(Distinct d1) {
		List<SelectExpressionItem> l1 = d1.getOnSelectItems();
		TreeMap<String, Integer> retval = new TreeMap<String, Integer>();
		for (int i = 0; i < l1.size(); i++) {
			String alias = l1.get(i).getAlias();
			if (alias != null || !alias.equals("")) {
				retval.put("Distinct_" + alias, 1);
			}
		}
