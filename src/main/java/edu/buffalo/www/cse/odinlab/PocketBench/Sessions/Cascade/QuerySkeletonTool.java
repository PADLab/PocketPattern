...
public class QuerySkeletonTool {
	
	public static ArrayList<Statement> getQuerySkeleton(ArrayList<Statement> query_set) {
		ArrayList<Statement> skeletonList = new ArrayList<Statement>();
		for (Statement query : query_set) {
			boolean foundSameSyntaxTree = false;
			for (int i = 0; i < skeletonList.size(); ++i) {
				if (compareStatement(query, skeletonList.get(i)) == true) {
					foundSameSyntaxTree = true;
				}
			}
			if (foundSameSyntaxTree == false) {
				skeletonList.add(query);
			}
		}
		return skeletonList;
	}
